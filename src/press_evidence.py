...
def theme_intro_from_evidence(
    ...
    if theme and theme != "기타 정부·R&D":
        return (
            f"당월 {actor_text} 발표 중 keywords.txt 섹션「{theme}」 키워드의 "
            f"원문 직접 언급은 제한적임. 보도자료에 명시된 범위만 정리함."
        )
    return (
        f"당월 {actor_text} 등에서 모니터링 키워드 외 국내 R&D·정책 신호가 확인됨. "
        f"원문 근거가 있는 항목만 반영함."
    )
