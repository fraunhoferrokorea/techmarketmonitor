...
<div class="layout">
  ...
  <main class="panel main" id="main">
    ...
    <div class="report-wrap" id="config-wrap" hidden>
      ...
      <details class="config-token" id="token-panel" open>
        <summary>Personal Access Token 입력 (Contents: Write)</summary>
        <div class="row">
          <input type="password" id="gh-token" placeholder="ghp_… 또는 fine-grained token" autocomplete="off"/>
          <button type="button" class="btn primary" id="btn-token-commit">토큰으로 GitHub에 저장</button>
...
function syncTokenUi() {{
  const ready = !!sessionStorage.getItem(TOKEN_KEY) || !!(qs("gh-token").value || "").trim();
  const el = qs("token-status");
  if (el) el.textContent = ready ? "토큰 상태: 준비됨 (이 탭 세션)" : "토큰 상태: 없음 — 아래에 입력하세요";
  const panel = qs("token-panel");
  if (panel && !sessionStorage.getItem(TOKEN_KEY)) panel.open = true;
}}
...
async function loadReport(href, title) {{
  ...
  try {{
    ...
    const res = await fetch(href + bust + "v=" + Date.now(), {{ cache: "no-store" }});
...
function enhanceReport(root) {{
  ...
  for (const el of kids) {{
    ...
    if (el.tagName === "P" && /날짜|기록자|총 항목|신뢰도|기간|생성일|발행|분석 항목/.test(el.textContent)) {{
      metaLines.push(el);
    }}
  }}
  if (metaLines.length) {{
  ...
  // Keep monitoring-keyword line as a full paragraph (top-5 + 등), not a truncated chip.
  root.querySelectorAll("p").forEach(p => {{
    if (/^모니터링 키워드/.test((p.textContent || "").trim())) {{
      p.classList.add("kw-header");
    }}
  }});
}}
...
qs("btn-token-commit").addEventListener("click", commitCurrent);
qs("gh-token").addEventListener("input", syncTokenUi);
