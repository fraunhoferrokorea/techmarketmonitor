...
국내 RSS·부처 보도자료·PACST 등에서 기사·보도자료를 수집하고(정부 URL은 본문·PDF 보강), **해외 기사 제외** + `keywords.txt` 전체·**정부·R&D 타깃**·**비-R&D 제외** 규칙으로 필터링한 뒤 LLM으로 **투자 주체·팩트/인용·위탁 연구 니즈** 중심 요약하여 **데일리 R&D 인텔리전스 로그**와 **월간 Markdown 보고서**(`monthly_YYYY-MM.md`)를 생성합니다.
...
```
매일:  국내 RSS·부처 HTML 수집 → 본문·PDF 보강(정부 URL) → Korea-scope + 비-R&D 제외 + keywords 전체 필터 → R&D·투자 신호 우선 LLM 요약(적합도 1–5, 팩트/의견 분리)
       → SQLite 저장 → daily_YYYY-MM-DD.md (R&D 기회 스캔 표) → Git push / Pages
매월:  daily_*.md/DB → R&D 적합 4점 이상 + 키워드 직접/간접 → monthly_YYYY-MM.md (키워드 섹션 기준)
```
...
- `#`으로 시작하는 줄과 빈 줄은 무시
- `# ── 섹션명 ──` 형태 헤더는 **월간 §3 분야 버킷** (`load_keyword_groups`)으로 사용
- **소문자로 정규화** 후 제목·요약·출처명에서 **부분 문자열 매칭**
- 이 파일만 수정하면 다음 실행부터 필터·분석 키워드 변경
- **전체 키워드 줄**이 수집 필터(`filter_keywords`)와 분석(`analysis_keywords`)에 **동일하게** 사용됨 (과거 top-3/top-5 구분 폐기)

| 구분 | keywords.txt 위치 | 용도 |
|------|-------------------|------|
| 전체 목록 | 모든 키워드 줄 | 수집 필수 매칭 · LLM `keyword_relevance` · 데일리/월간 Executive Summary · `matched_keywords` |
| 섹션 헤더 | `# ── … ──` | 월간 Opportunities 분야 라벨 |
...
| 구분 | 예시 소스 |
|------|-----------|
| 정부·공공기관 | 과기정통부, 산업통상부, 중기부, KISTEP, KIPO, 한국전력 |
...
# 저장된 DB 행에 현재 수집 필터(keywords.txt 전체 + 비-R&D 제외) 재적용 후 md 재생성 (LLM 없음)
...
| `daily-refilter` | `--from`, `--to` | (필수) | DB 저장분에 **현재 수집 필터**(keywords.txt 전체 + `is_excluded_rd_news`) 재적용·불합격 행 삭제·md 재생성 (LLM 없음) |
| `summarize-plan` | `--file`, `--save-text` | — | 정부 계획 PDF map-reduce 요약 → `output/plans/{stem}_summary.md` (데일리 수집 범위 외) |
| `show-config` | — | — | `keywords_path`, `analysis_keywords`, `filter_keywords`, DB·reports 경로 JSON 출력 |
...
- `sources.md` 국내 RSS·HTML 보도 피드를 순회, 실패한 fetcher는 로그 후 건너뜀
...
### 3. 키워드·정부 타깃·국내 범위·비-R&D 필터

구현: `src/filter.py` — `passes_collection_filter()`, `filter_articles()`  
비-R&D 제외: `src/rd_targeting.py` — `is_excluded_rd_news()`

1. **국내 범위:** `is_domestic_news()` — 해외-only·외신 재인쇄·비한국 URL 제외 (`korea_scope.py`)
2. **비-R&D 제외** (`is_excluded_rd_news`) — 모니터링 키워드가 있어도 아래면 **수집·적합도에서 드롭**:
   - **현장체험·진로·비교과** (`is_non_rd_program_news`) — 학생 견학·채용연계 인턴십 등
   - **개인 자격·합격 인터뷰** (`is_career_credential_news`) — 기술사·기사·자격증 합격/취득 수기·최연소 합격 등 (예: 제목에 「전력계통」이 있어도 발송배전기술사 합격 인터뷰는 제외)
   - **연구성과-only** (`is_research_outcome_without_investment_signal`) — 투자·예산·공모·위탁 신호 없는 학술·역학 결과 보도  
   - **예외(유지):** 자격 **제도 개편** 정책, R&D/예산/실증 신호가 본문에 있으면 override (`_NON_RD_EDUCATION_RD_OVERRIDE` / `_RD_FUNDING_SIGNAL`)
3. **필수 키워드 (`filter_keywords` = keywords.txt 전체):** 제목·요약·출처에 1개 이상 없으면 제외  
   - **예외:** **정부·R&D 타깃** (`passes_gov_collection_exception`) — 국가계획·MOU·R&D 공고 등 + 에너지·그리드 도메인 힌트(또는 Fraunhofer 명시)
4. **매칭 태깅:** 통과 후 전체 키워드로 `matched_keywords` 부여 (gov-target만 매칭 시 `정부·R&D 타깃` 라벨)
5. URL 중복 제거 (같은 실행 내)

> **목적:** 키워드만 맞고 **투자·위탁 R&D 신호가 없는** 교육·자격·성과 보도를 걸러 Fraunhofer 타겟팅 노이즈를 줄임. 규칙 변경 후 기존 DB 정리는 `daily-refilter` (LLM 없음) 또는 `daily-reprocess` (LLM) 사용.
...
- DB에 저장된 각 행을 `_row_to_raw_article()`로 복원 후 `passes_collection_filter(keywords, filter_keywords)` 재검사 (`is_excluded_rd_news` 포함)
- 불합격 URL은 `delete_by_url()` — **LLM 재호출 없음**
- 남은 행으로 `rebuild_markdown_from_db()` → md 재생성. 전부 삭제되면 해당 날짜 md 제거(또는 빈 플레이스홀더)
- **수집 규칙·비-R&D 제외** 변경 후 **기존 DB만 정리**할 때 사용 (`daily-reprocess`보다 빠름·저렴)
...
**분석 기준 키워드:** 전력계통 · 파워그리드 · smart grid · … (keywords.txt 전체)
...
- **`국내 R&D 타겟 시사점 (프라운호퍼)`** — `rd_targeting.build_daily_rd_insights()`로 국내 투자 주체 요약
...
- **자료유형:** 기사 / 보도자료 / 논문 / 보고서(시장조사) / 기타
- **출처:**
- **저자/발행기관:**
- **발행일:** YYYY-MM-DD
- **링크/DOI:**
- **요약:**
  - 팩트 bullet (가능하면 「」 원문 직접 인용, 명사형 `-함`/`-었음`)
  - (의견) keyword_relevance·해석은 **별도 줄** (팩트와 혼재 금지)
- **R&D 타겟팅 (프라운호퍼):** (해당 시)
  - **투자 주체:** … (팩트만)
  - **투자 목적:** … (팩트만)
  - **팩트 근거 / 원문 인용:** …
...
- `보도자료` = `_material_type() == "보도자료"` (출처명에 보도자료 / 정부·.go.kr 등)
- `기사` = 그 외 (보고서(시장조사) 포함)
...
| 출처명에 `보도자료` 또는 정부·.go.kr·MOTIE 등 | 보도자료 |
| Gartner/IDC/McKinsey/Statista·공공 리서치 출처 | 보고서(시장조사) |
| 그 외 | 기사 |
...
> 파일 하단 **태그 분류체계·신뢰도 기준 표는 더 이상 자동 첨부하지 않음** (Pages/md 노이즈 감소).
...
후처리: `src/fact_grounding.py` — 원문에 없는 수치·기관명 환각 차단

### 목적

프라운호퍼 한국 사무소 **R&D·기술협력 타겟팅**. 순수 기술 설명이 아니라 **투자 주체·예산·위탁 연구 니즈·접근 전략** 중심. 지리적 초점: **대한민국**(해외-only는 `국내 주체: 해당 없음(해외)`).

### 팩트 / 인용 / 의견 분리 (CRITICAL)

1. **없는 내용 금지** — 원문에 없는 수치·기관·사업명·일정 생성 금지. 불확실하면 `명시 없음` / `해당 없음` / `팩트 부족으로 판단 보류`
2. **직접 인용 우선** — `fact_basis`·개요에 원문 핵심을 「」로 붙임 (의역보다 부분 문자열 인용)
3. **의견 분리** — 위탁 연구 니즈·접근 전략·`keyword_relevance`·`rd_proposable_area`만 분석가 판단. 문장 앞에 **`(의견)`**. 개요·투자 주체·투자 목적·`ko_one_liner`·`fact_basis`에는 의견 금지

### JSON 출력 스키마

```json
{
  "summary": "1문장 한국어 R&D·투자 헤드라인(원문 팩트만). 반드시 출처: <url> 포함",
  "ko_summary_steps": [
    "**개요:** … (팩트, 가능하면 「」 인용)",
    "**투자 주체:** …",
    "**투자 목적:** …",
  ...
  "ko_one_liner": "Executive Summary 표용 5W1H 1문장 (70~150자, -함/-었음, 팩트만)",
  "key_trends": ["원문 기술·투자 동향 1", "키워드 2"],
  "fact_basis": "예산·일정·규모 「」 직접 인용. 없으면 명시 없음",
  "rd_proposable_area": "(의견) 제안 가능 R&D 영역. 근거 없으면 해당 없음",
  "keyword_relevance": "(의견) keywords.txt 전체와 이 기사의 연관 (2문장 이내)"
}
```
...
2. **keyword_relevance**: keywords.txt **전체**와 **이 기사**의 연결만. `(의견)` 접두. 키워드별 문단 분리(`OO와 관련하여`)·정의·일반론 금지. 원문 연결 없으면 `(의견) 모니터링 키워드와 직접·간접 관련 없음.`
3. **사실 정확성**: 수치·날짜·기업명 100% 원문 일치. 없는 내용 추론 금지 (`fact_grounding.sanitize_summarized_article`)
...
- **포함 기준:** `compute_rd_match_score()` ≥ `MONTHLY_RD_MIN_SCORE`(기본 **4**) **그리고** 키워드 관련도 **직접/간접**만 (§1–§5 본문). `약함`/`없음`은 스코어카드 외 본문에서 제외
- **적합도 산정:** Fraunhofer 협력 신호(투자 주체·니즈·제안 R&D) **기본 점수** + `keywords.txt` **전체** 관련도 보정  
...
- **보도자료 근거:** `press_evidence.py` — 부처 보도 HTML·첨부에서 키워드 attestation·「」 인용 후 §3/§4 반영
- C급 신뢰도 항목은 `prepare_logs_for_monthly_rd()`에서 제외
- `is_excluded_rd_news` 항목은 월간 적합도·집계에서도 제외
...
1. **Executive Summary** — 당월 국내 R&D·투자 트렌드 (모니터링 키워드·정부 신호, 문단 분리)
2. **컨텍스트 중요도 상위** — keywords.txt 대비 **직접·간접** 관련 이슈
3. **Opportunities (분야별 R&D 기회)** — 분야 라벨은 `keywords.txt` **섹션 헤더**. 항목은 육하원칙 짧은 팩트, **투자 주체만 볼드**
4. **주요 R&D 타겟 상세** — 투자 주체·목적·팩트 근거·원문 인용·원문 확인 키워드·출처 (**팩트만**; 위탁/제안/접근 전략·관련도 미표시)
...
- 소스에 없는 수치·기관명 **추가 금지**. 가능하면 보도자료 「」 인용
- `_factcheck_structured`로 키워드 나열 환각·중복 정책 보도 정리
...
│   │   └── test_pacst.py
│   ├── filtering/
│   │   ├── test_catchup_holes.py
│   │   ├── test_daily_relevance_split.py
...
│       ├── test_daily_reference_links.py
│       ├── test_fact_grounding.py
│       ├── test_keyword_highlight.py
│       ├── test_press_evidence.py
...
│   ├── press_evidence.py         # 부처 보도 attestation·「」 인용
│   ├── fact_grounding.py         # LLM 환각 차단 후처리
...
│   ├── rd_targeting.py           # R&D 타겟팅·비-R&D 제외·일일 시사점
...
│   ├── summarizer.py             # LLM 요약 + 한국어·팩트/의견 규칙
│   ├── filter.py                 # keywords.txt 매칭 + is_excluded_rd_news
│   ├── storage.py                # SQLite DailyLogStore
│   ├── monthly.py                # daily md 월간 집계 + md 정리
│   ├── report_generator.py       # LLM JSON 합성 + Word 빌드 (EN/KO)
│   ├── config.py                 # Settings, load_sources() (sources.md 우선)
│   ├── models.py                 # RawArticle, FilteredArticle, SummarizedArticle
│   └── fetchers/
│       ├── registry.py
│       ├── rss.py
│       ├── html_boards.py        # 부처·KIAT/NRF/KAIA/KRIT/한전 등 HTML 보도
│       ├── pacst.py              # PACST 게시판 (catch-up)
│       └── base.py
├── keywords.txt
├── sources.md                    # 운영 수집 소스 (클릭 가능 링크)
├── email_recipients.txt          # 이메일 수신자
├── docs/                         # GitHub Pages OUTPUT 뷰어
├── requirements.txt
├── .env / .env.example
...
- src/filter.py: keywords.txt 전체 매칭 + is_gov_target → 정부·R&D 타깃 통과 + is_excluded_rd_news
- src/policy_priority.py: gov_target_score, 정부·R&D 타깃 탐지
- src/summarizer.py + focused_summarize.py: LLM JSON (R&D 5단계 + ko_one_liner + fact_basis + keyword_relevance, 팩트/의견 분리)
- src/rd_targeting.py: R&D 타겟팅 블록, is_excluded_rd_news(현장체험·자격합격·성과-only), build_daily_rd_insights
- src/fact_grounding.py + press_evidence.py: 환각 차단, 보도자료 attestation
...
- daily-refilter --from YYYY-MM-DD --to YYYY-MM-DD: DB에 현재 수집 필터(keywords 전체 + is_excluded_rd_news) 재적용·md 재생성 (LLM 없음)
...
## 설정
- keywords.txt: 전체 줄 = 수집 필터 + LLM·월간 분석 (섹션 헤더 = 월간 §3 분야). top-3/top-5 구분 없음
- sources.md (운영) + config/sources.yaml 이름 매칭 폴백, email_recipients.txt
...
{
  "summary": "1문장 한국어 R&D·투자 헤드라인(원문 팩트만). 반드시 출처: <url> 포함",
  "ko_summary_steps": ["**개요:** ...", "**투자 주체:** ...", "**투자 목적:** ...", "**위탁 연구 니즈:** (의견) ...", "**접근 전략:** (의견) ..."],
  "ko_one_liner": "Executive Summary 5W1H 1문장 (70~150자, -함/-었음, 팩트만)",
  "key_trends": ["..."],
  "fact_basis": "예산·일정 「」 직접 인용. 없으면 명시 없음",
  "rd_proposable_area": "(의견) 제안 R&D. 근거 없으면 해당 없음",
  "keyword_relevance": "(의견) keywords.txt 전체와 이 기사의 연관"
}
en_summary_steps: 빈 배열 저장 (데일리 md는 한국어만)
팩트/인용/의견 분리: 없는 내용 금지; 「」 인용 우선; 의견은 (의견) 접두만. is_excluded_rd_news로 현장체험·자격합격 인터뷰·연구성과-only 제외.
fact_grounding.sanitize_summarized_article 후처리.

### 한국어 생성 규칙 (CRITICAL — 반드시 준수)
1. 직역 금지: en_summary_steps의 번역이 아님. 한국 독자용으로 독립 재서술
2. keyword_relevance: keywords.txt 전체와 이 기사의 연결만. (의견) 접두. 키워드별 문단 분리(OO와 관련하여) 금지. 키워드 정의·일반론 금지
...
| 2026-07-08~ | **비-R&D 제외:** 현장체험·진로 프로그램, 투자 신호 없는 연구성과-only (`is_excluded_rd_news`) |
| 2026-07-08~ | **팩트 근거 강화:** `fact_grounding.py` 환각 차단; 데일리 요약 팩트-only |
| 2026-07-17~ | **GitHub Pages OUTPUT 뷰어** (`docs/`): 데일리·월간·Config(keywords/sources) |
| 2026-07-20 | **키워드 전체 사용:** 수집·분석에서 top-3/top-5 폐기 → `keywords.txt` 전 줄 |
| 2026-07-20 | **소스 전환:** 운영 기준 `sources.md`; korea.kr 아카이브 제거 → 부처·기관 HTML 보도(`html_boards.py`) |
| 2026-07-20~ | **데일리 md:** 하단 태그 분류표 제거; 키워드 하이라이트; 기사/보도자료/논문 건수 분리 |
| 2026-07-21~ | **팩트/인용/의견 분리:** 요약·월간에 「」 인용 우선, `(의견)` 접두, 월간 §4 팩트-only |
| 2026-07-21~ | **월간:** 보도자료 attestation(`press_evidence`), 키워드 직접/간접만 본문, §3=keywords.txt 섹션 |
| 2026-07-24 | **개인 자격·합격 인터뷰 제외:** `is_career_credential_news` (기술사·자격증 합격 수기 등). 키워드 히트만으로는 수집하지 않음 |
