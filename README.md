...
```
매일:  (선택) md/DB 불일치 수리 → RSS 수집 → 날짜 필터(캐치업=KST 캘린더일 / live=24h) → keywords.txt 매칭
       → LLM 요약 → SQLite 저장 → daily_YYYY-MM-DD.md 생성 → (로컬) Git push
...
### `sources.txt` — RSS 소스 (한 줄 = 소스 1개)

- 형식: `이름 | RSS URL | 카테고리` (파이프 `|` 구분)
- `#`으로 시작하는 줄과 빈 줄은 무시
- 이 파일만 수정하면 다음 실행부터 수집 소스 변경
- `sources.txt`가 없거나 비어 있으면 `config/sources.yaml`을 폴백으로 사용
...
| `korean` | 과기정통부, 정책브리핑, KISTEP, KIPO, 연합뉴스, 뉴시스, 한국경제, 전자신문, ZDNet, 조선일보 IT/과학 |

`src/fetchers/registry.py`가 항목마다 `RSSFetcher` 1개 생성.

### `config/sources.yaml` — RSS 소스 (레거시 / 폴백)

`sources.txt`와 동일한 7개 카테고리·36개 피드. `sources.txt`를 쓰지 않을 때만 참조됩니다.
...
| 관련도 | 항목 | 한 줄 요약 |
|--------|------|-----------|
| **직접** | [기사 제목…](#anchor-slug) | 구체적 사실문장 → **[키워드]** 직접 연관 |
| **간접** | [기사 제목…](#anchor-slug) | … → **[키워드]** 간접 연관 (이유) |
...
- `ko_summary_steps`에서 사실문장 추출 (`keyword_relevance` boilerplate보다 우선)
- 지시대명사·정의문·“중요성 강조” 등 **모호 패턴(`_VAGUE_PATTERNS`)** 필터
- 관련도 **직접/간접**만 표에 포함, `weak`/`none`은 생략 (하단에 건수 표기)
- 항목 제목은 `[짧은 제목](#slug)` — 본문 `### N. 제목` 앞 `<a id="slug"></a>`와 연결 (GitHub md 렌더링용)
...
- **링크/DOI:**
- **요약 (영문 원문):**
  - **Overview:** … (en_summary_steps 상위 3개)
- **요약 (한국어 번역):**
  - **개요:** … (ko_summary_steps, -함/-임 명사형)
  - 마지막 줄: `(해석) ...`
- **신뢰도:** A / B / C (또는 B (프리프린트, 동료심사 전))
- **태그:** #태그1 #태그2 (최대 4개)
- **비고:** 분석 키워드 · 키워드 관련도(직접/간접) · 매칭 키워드
```
...
├── src/
│   ├── main.py                   # CLI (daily, daily-catchup, daily-repair, daily-reprocess, monthly, schedule)
│   ├── catchup.py                # 빠진 날짜 순차 실행 + repair 선행
│   ├── daily_sync.py             # md/DB 불일치 수리·재처리
│   ├── daily_markdown_loader.py  # daily md → 월간 집계용 dict
│   ├── scheduler_state.py        # last_completed_log_date, remove_report()
│   ├── pipeline.py               # fetch → filter(24h|calendar-day) → summarize → store → md
│   ├── daily_report.py           # Markdown 리서치 로그 (Executive Summary, 이중언어)
...
- src/pipeline.py: fetch→(live: 24h filter | catch-up: KST calendar-day filter)→keyword filter→관련성 정렬→MAX_ARTICLES_PER_RUN(30) cap→URL dedup→summarize→store→save_daily_report. stale md 자동 삭제
- src/daily_report.py: daily_YYYY-MM-DD.md (Executive Summary 표+앵커링크, EN/KO 이중 요약, 자료유형/신뢰도/태그 자동 분류, 모호문장 필터)
- src/daily_sync.py: md/DB 불일치 scan/repair, reprocess_range
...
## 설정
- keywords.txt, config/sources.yaml, email_recipients.txt
- .env: OPENAI_*, MODEL_NAME, DATABASE_PATH, REPORTS_OUTPUT_DIR=reports/, LOG_LEVEL, MAX_ARTICLES_PER_RUN, SUMMARIZER_REQUEST_DELAY, DAILY_LOG_RECORDER, EMAIL_*(준비 중)
...
Executive Summary: 키워드 표, 관련도|항목|한줄요약 테이블, GitHub 앵커 링크, 모호문장 필터
항목: EN 원문 + KO 번역, anchor slug, 비고에 키워드 관련도
